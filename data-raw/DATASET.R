...
library(magrittr)
set.seed(35397290)
normal_data <- read.table("normal_an10_full_correct.txt",header = TRUE,row.names = 1) %>% as.matrix() %>% t()
sick_data <- read.table("sick_an10_full.txt",header = TRUE, row.names = 1) %>% as.matrix() %>% t()
